...
public class KeyAuthenticationConverter implements ServerAuthenticationConverter {
    ...
    public KeyAuthenticationConverter(
            @Value("${VALID_API_KEYS}") String keys) {
    ...
    /**
     * Returns a Mono of Authentication if the API key is present in the headers and is valid, otherwise returns an empty Mono.
     * @param exchange
     * @return
     */
    @Override
    public Mono<Authentication> convert(ServerWebExchange exchange) {
        return Mono.justOrEmpty(exchange)
                .flatMap(ex -> Mono.justOrEmpty(
                        ex.getRequest().getHeaders().getFirst(API_KEY_HEADER_NAME)
                ))
                .doOnNext(key -> System.out.println("Incoming API key: " + key))
                .filter(validKeys::contains)
                .map(key -> new UsernamePasswordAuthenticationToken(
                        "api-user", key, List.of()
                ));
    }
